...
public class NPCController_ : MonoBehaviour
{
    [Header("Setup")]
    ...
    // Using byte instead of bool for better memory efficiency in Burst-compiled code
    ...
    void OnDestroy()
    {
        ...
        if (npcCanAttackState.IsCreated)
        {
            npcCanAttackState.Dispose();
        }

        if (enemyCanAttackState.IsCreated)
        {
            enemyCanAttackState.Dispose();
        }
    }
}